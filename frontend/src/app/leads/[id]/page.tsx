    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">
          AI Analysis - Confidence: {formatConfidence(lead.confidence)}
        </h2>
        {lead.ai_summary && (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {(["new", "contacted", "qualified", "closed", "lost"] as const).map(s => {
            ...
            return (
              <span key={s} className={`px-4 py-2 rounded-lg text-sm font-medium border ${isActive ? cfg.color + " " + cfg.bg : "text-gray-500 bg-white border-gray-200"}`}>
                {cfg.label}
              </span>
            );
          })}
